...
USER_HEADERS = {"X-User-ID": "report-user"}
...
async def _buy(client, stock_id, units, price, date="2024-08-01", fees=0):
    payload = {"stock_id": stock_id, "type": "Buy", "units": units,
               "price": price, "fees": fees, "transaction_date": date}
    resp = await client.post("/api/transactions/", json=payload, headers=USER_HEADERS)
    assert resp.status_code == 201
    return resp.json()


async def _sell(client, stock_id, units, price, date="2025-01-01", fees=0):
...
class TestHoldings:
    ...
    async def test_partial_sell_reduces_units(self, client):
        stock = await _create_stock(client)
        await _buy(client, stock["id"], units=10, price=25, date="2024-08-01")
        await _sell(client, stock["id"], units=4, price=30, date="2024-09-01")
        resp = await client.get("/api/reports/holdings", headers=USER_HEADERS)
        holdings = resp.json()["holdings"]
        assert len(holdings) == 1
    ...
    async def test_full_sell_excluded_from_holdings(self, client):
        stock = await _create_stock(client)
        await _buy(client, stock["id"], units=10, price=25, date="2024-08-01")
        await _sell(client, stock["id"], units=10, price=30, date="2024-09-01")
        resp = await client.get("/api/reports/holdings", headers=USER_HEADERS)
        assert resp.json()["holdings"] == []
    ...
    async def test_as_of_date_excludes_future_transactions(self, client):
        stock = await _create_stock(client)
        await _buy(client, stock["id"], units=10, price=25, date="2024-08-01")
        await _buy(client, stock["id"], units=5, price=30, date="2025-06-01")
        resp = await client.get("/api/reports/holdings",
                                params={"as_of": "2024-12-31"}, headers=USER_HEADERS)
        holdings = resp.json()["holdings"]
    ...
    async def test_avg_cost_basis_weighted(self, client):
        stock = await _create_stock(client)
        # 10 @ $20 + 10 @ $30 → avg = $25
        await _buy(client, stock["id"], units=10, price=20, date="2024-08-01")
        await _buy(client, stock["id"], units=10, price=30, date="2024-09-01")
        resp = await client.get("/api/reports/holdings", headers=USER_HEADERS)
        holdings = resp.json()["holdings"]
        assert holdings[0]["avg_cost_basis"] == pytest.approx(25)
...
class TestCapitalGains:
    ...
    async def test_no_sells_returns_zero(self, client):
        ...
        resp = await client.get("/api/reports/capital-gains",
                                params={"fy": 2024}, headers=USER_HEADERS)
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_simple_gain(self, client):
        ...
        await _sell(client, stock["id"], units=10, price=30, date="2025-01-01")
    ...
    async def test_sell_with_fees_reduces_proceeds(self, client):
        stock = await _create_stock(client)
        await _buy(client, stock["id"], units=10, price=20, date="2024-08-01")
        await _sell(client, stock["id"], units=10, price=30, date="2025-01-01", fees=9.95)
    ...
    async def test_loss_is_negative(self, client):
        stock = await _create_stock(client)
        await _buy(client, stock["id"], units=10, price=30, date="2024-08-01")
        await _sell(client, stock["id"], units=10, price=20, date="2025-01-01")
    ...
    async def test_sells_in_wrong_fy_excluded(self, client):
        stock = await _create_stock(client)
        await _buy(client, stock["id"], units=10, price=20, date="2023-08-01")
        # This sell is in FY2023 (Jan 2024), not FY2024
        await _sell(client, stock["id"], units=5, price=30, date="2024-01-01")
    ...
    async def test_multiple_stocks_reported_separately(self, client):
        ...
        await _buy(client, bhp["id"], units=10, price=20, date="2024-08-01")
        await _buy(client, cba["id"], units=5, price=100, date="2024-08-01")
        await _sell(client, bhp["id"], units=10, price=30, date="2025-01-01")
        await _sell(client, cba["id"], units=5, price=80, date="2025-01-01")
